...
class MultiHeadFlowAwareAttention(nn.Module):
    ...
    def forward(self, node_feat, Coord, edge_index, edge_weight, M):
        ...
        for i in range(batch_size):
            ...
            A_raw[i, src[start_idx:end_idx], dst[start_idx:end_idx], :] = attention_scores[start_idx:end_idx, :]
...
class GraphModel(nn.Module):
    def __init__(self, input_steps=20, d_in=64, d_out=1, num_heads=8, codebook_size = 100):
    ...
    def forward(self, data, t_input):
        ...
        # print(type(x), type(edge_index), type(edge_weight))
        ...
        node_feat = self.ode_update(A, node_feat, t_input, self.M)  # [1, batch_size, M, d_in]
        # print(node_feat.shape)
        output = self.decoder(node_feat.reshape(-1, self.d_in))  # [batch_size*M, d_out]
        # print(output.shape)
        return output
